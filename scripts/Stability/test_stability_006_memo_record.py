...
class TestMemoRecord():
    ...
    @pytest.mark.parametrize(("name", "label"), analyze_file("record_data", "test_memo_record"))
    def test_memo_record(self, name, label):
        self.page.local_record_page.click_memo()
        for i in range(3):
            time.sleep(602)
